...
def filter_group_data(batch_list : List[Dict],
                        ...
                        config,
                        last_try: bool = False,
                        env_episode_rewards: np.ndarray = None,
                        ):
    """
    Dynamic Sampling:
    ...
    """
    if last_try:
        return batch_list, episode_rewards, env_episode_rewards, episode_lengths, success, traj_uid, tool_callings
    ...
    if env_episode_rewards is not None:
        env_episode_rewards = env_episode_rewards[keep_indices]
