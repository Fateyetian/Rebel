...
def compute_gigpo_outcome_advantage(token_level_rewards: torch.Tensor,
    ...
    step_group_uids, group_stats = build_step_group(anchor_obs, index, enable_similarity, similarity_thresh)
    ...
    return scores, scores, group_stats
...
def build_step_group(anchor_obs: np.array, index: np.array, enable_similarity: bool = False, similarity_thresh: float = 0.95, summarize: bool = False):
    ...
    group_size_arr = np.array(group_size)
    group_stats = {
        'num_groups': len(group_size_arr),
        'mean_group_size': float(np.mean(group_size_arr)),
        'median_group_size': float(np.median(group_size_arr)),
        'min_group_size': int(np.min(group_size_arr)),
        'max_group_size': int(np.max(group_size_arr)),
        'std_group_size': float(np.std(group_size_arr)),
        'single_sample_ratio': float(np.sum(group_size_arr == 1) / len(group_size_arr)),
    }
    print(f"Avg size of step-level group: {group_stats['mean_group_size']:.2f}, singleton ratio: {group_stats['single_sample_ratio']:.3f}")
    return step_group_uids, group_stats
