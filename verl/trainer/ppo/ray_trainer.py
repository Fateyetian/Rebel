...
from verl.trainer.ppo.gigpo import core_gigpo
...
def apply_invalid_action_penalty(data: DataProto, invalid_action_penalty_coef=float):
    ...
    metrics = {
        ...
        'episode/invalid_action_rate': 1.0 - valid_action_ratio,
    }
    return data, metrics
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, step_advantage_w=1.0, gigpo_mode="mean_std_norm", gigpo_enable_similarity=False, gigpo_similarity_thresh=0.95, **kwargs):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns, gigpo_group_stats = core_gigpo.compute_gigpo_outcome_advantage(
        ...
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
        for k, v in gigpo_group_stats.items():
            data.meta_info[f'gigpo_{k}'] = v
    elif adv_estimator == AdvantageEstimator.ReBel:
        from verl.trainer.ppo.rebel.core_rebel import compute_rebel_advantage
        ...
        gamma = float(data.meta_info.get('rebel_gamma', 0.95))
        ...
        cumulative_belief_states = data.non_tensor_batch.get('cumulative_belief_state', None)
        gt_phases = data.non_tensor_batch.get('gt_phase', None)
        alpha_belief_ep = float(data.meta_info.get('rebel_alpha_belief_ep', 0.5))
        ...
        advantages, returns, adv_details = compute_rebel_advantage(
            ...
            gamma=gamma,
            cumulative_belief_states=cumulative_belief_states,
            gt_phases=gt_phases,
            alpha_belief_ep=alpha_belief_ep,
        )
        ...
        if task_types is not None:
            ...
            for task in unique_tasks:
                task_mask = _torch.tensor([t == task for t in task_types], device=advantages.device)
                task_mask_expanded = task_mask.unsqueeze(1).expand_as(advantages)
                combined_mask = task_mask_expanded & response_mask.bool() if response_mask is not None else task_mask_expanded
                task_advs = advantages[combined_mask]
                if len(task_advs) > 0:
                    task_short = task.replace('pick_', '').replace('_then_place_in_recep', '').replace('_obj', '')
                    data.meta_info[f'rebel/adv_mean_{task_short}'] = task_advs.mean().item()
                    data.meta_info[f'rebel/adv_std_{task_short}'] = task_advs.std().item() if len(task_advs) > 1 else 0.0

        for key in adv_details:
            if key.startswith('rebel/task_weight_') or key.startswith('rebel/use_task_weighting'):
                data.meta_info[key] = adv_details[key]

        # Forward all scalar adv_details to meta_info for downstream logging
        _SKIP_KEYS = {'episode_advantages', 'step_advantages', 'belief_group_stats'}
        for key, val in adv_details.items():
            if key not in _SKIP_KEYS and isinstance(val, (int, float)):
                data.meta_info[f'rebel_scalar_{key}'] = val
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator in [AdvantageEstimator.GiGPO]:
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        # Inject rebel config into batch.meta_info so compute_advantage can read Hydra overrides
                        if self.config.algorithm.adv_estimator in [AdvantageEstimator.ReBel]:
                            rebel_cfg = getattr(self.config.algorithm, 'rebel', None)
                            if rebel_cfg is not None:
                                batch.meta_info['rebel_belief_granularity'] = str(getattr(rebel_cfg, 'belief_granularity', 'subgoal'))
                                batch.meta_info['rebel_step_advantage_w'] = float(getattr(rebel_cfg, 'step_advantage_w', 1.0))
                                batch.meta_info['rebel_mode'] = str(getattr(rebel_cfg, 'mode', 'mean_norm'))
                                batch.meta_info['rebel_task_aware_grouping'] = bool(getattr(rebel_cfg, 'task_aware_grouping', False))
                                batch.meta_info['rebel_per_task_normalization'] = bool(getattr(rebel_cfg, 'per_task_normalization', False))
                                batch.meta_info['rebel_conditional_norm'] = bool(getattr(rebel_cfg, 'conditional_norm', True))
                                batch.meta_info['rebel_min_samples_for_norm'] = int(getattr(rebel_cfg, 'min_samples_for_norm', 10))
                                batch.meta_info['rebel_min_std_for_norm'] = float(getattr(rebel_cfg, 'min_std_for_norm', 0.1))
                                batch.meta_info['rebel_use_task_weighting'] = bool(getattr(rebel_cfg, 'use_task_weighting', False))
                                batch.meta_info['rebel_task_success_rates'] = self.task_success_rates
                                batch.meta_info['rebel_weight_alpha'] = float(getattr(rebel_cfg, 'weight_alpha', 2.0))
                                batch.meta_info['rebel_weight_min'] = float(getattr(rebel_cfg, 'weight_min', 0.3))
                                batch.meta_info['rebel_weight_max'] = float(getattr(rebel_cfg, 'weight_max', 3.0))
                                batch.meta_info['rebel_weight_baseline_sr'] = float(getattr(rebel_cfg, 'weight_baseline_sr', 0.85))
                                batch.meta_info['rebel_gamma'] = float(self.config.algorithm.gamma)
                                batch.meta_info['rebel_alpha_belief_ep'] = float(getattr(rebel_cfg, 'alpha_belief_ep', 0.5))
                ...
                # GiGPO grouping stats
                if self.config.algorithm.adv_estimator == AdvantageEstimator.GiGPO:
                    for _k in ('gigpo_num_groups', 'gigpo_mean_group_size', 'gigpo_median_group_size',
                               'gigpo_min_group_size', 'gigpo_max_group_size', 'gigpo_std_group_size',
                               'gigpo_single_sample_ratio'):
                        if _k in batch.meta_info:
                            metrics[f'gigpo/{_k[6:]}'] = batch.meta_info[_k]

                # ReBel-specific metrics forwarding
                _rebel_enabled = getattr(
                    getattr(self.config.algorithm, 'rebel', None), 'enable', False)

                # Per-component belief rewards + belief_accuracy: log whenever rebel is enabled
                # (covers A2 where adv_estimator=gigpo but rebel.enable=True)
                if _rebel_enabled:
                    import numpy as _np
                    for _comp in ('rebel_r_state', 'rebel_r_task', 'rebel_r_pred', 'rebel_r_format'):
                        if _comp in batch.non_tensor_batch:
                            _v = batch.non_tensor_batch[_comp].astype(float)
                            metrics[f'rebel/{_comp[6:]}_mean'] = float(_v.mean())
                            metrics[f'rebel/{_comp[6:]}_std']  = float(_v.std())
                    # belief_accuracy = r_state (object/state/visited F1 vs GT)
                    if 'rebel_r_state' in batch.non_tensor_batch:
                        metrics['rebel/belief_accuracy'] = float(
                            batch.non_tensor_batch['rebel_r_state'].astype(float).mean())
                    if 'rebel_belief_parsed' in batch.non_tensor_batch:
                        metrics['rebel/belief_parse_rate'] = float(
                            batch.non_tensor_batch['rebel_belief_parsed'].mean())
                    if 'rebel_is_repeat_action' in batch.non_tensor_batch:
                        metrics['rebel/repeat_action_rate'] = float(
                            batch.non_tensor_batch['rebel_is_repeat_action'].mean())

                if self.config.algorithm.adv_estimator == AdvantageEstimator.ReBel:
                    # 1. scalar adv_details from compute_advantage
                    for key, val in batch.meta_info.items():
                        if key.startswith('rebel_scalar_') and isinstance(val, (int, float)):
                            metrics[key.replace('rebel_scalar_', 'rebel/')] = val

                    # 2. belief group stats
                    for _k in ('rebel_num_groups', 'rebel_mean_group_size', 'rebel_median_group_size',
                               'rebel_min_group_size', 'rebel_max_group_size', 'rebel_std_group_size',
                               'rebel_single_sample_ratio'):
                        if _k in batch.meta_info:
                            metrics[f'rebel/{_k[6:]}'] = batch.meta_info[_k]

                    # 3. per-component reward distributions (already logged above via _rebel_enabled)

                    # 4. intrinsic reward (r_cons) stats
                    if 'rebel_intrinsic_reward' in batch.batch:
                        import torch as _torch
                        _rc = batch.batch['rebel_intrinsic_reward'].float()
                        metrics['rebel/r_cons_mean'] = float(_rc.mean())
                        metrics['rebel/r_cons_std'] = float(_rc.std())
                        metrics['rebel/r_cons_max'] = float(_rc.max())

                    # 5. episode/step advantage tensor stats (from meta_info tensors)
                    import torch as _torch
                    for _adv_key in ('episode_advantages', 'step_advantages'):
                        if _adv_key in batch.meta_info:
                            _t = batch.meta_info[_adv_key]
                            if isinstance(_t, _torch.Tensor):
                                metrics[f'rebel/{_adv_key}_mean'] = float(_t.mean())
                                metrics[f'rebel/{_adv_key}_std'] = float(_t.std())

                    # 6. Per-task belief accuracy and r_cons
                    ...
                    if ('rebel_r_state' in batch.non_tensor_batch and
                            'task_type' in batch.non_tensor_batch):
                        _r_state = batch.non_tensor_batch['rebel_r_state'].astype(float)
                        _task_types = batch.non_tensor_batch['task_type']
                        _r_cons_arr = None
                        if 'rebel_intrinsic_reward' in batch.batch:
                            _r_cons_arr = batch.batch['rebel_intrinsic_reward'].float().cpu().numpy()
                        for _task in _np2.unique(_task_types):
                            _mask = _task_types == _task
                            if _mask.sum() == 0:
                                continue
                            _short = _TASK_SHORT.get(str(_task), str(_task)[:8])
                            metrics[f'rebel/belief_acc_{_short}'] = float(_r_state[_mask].mean())
                            if _r_cons_arr is not None:
                                metrics[f'rebel/r_cons_{_short}'] = float(_r_cons_arr[_mask].mean())

                    # 7. Per-step-bucket belief accuracy (reveals intra-episode drift)
                    if ('rebel_r_state' in batch.non_tensor_batch and
                            'rebel_step_index' in batch.non_tensor_batch):
                        _r_state = batch.non_tensor_batch['rebel_r_state'].astype(float)
                        _step_idx = batch.non_tensor_batch['rebel_step_index'].astype(int)
                        for _lo, _hi in [(0, 5), (5, 10), (10, 20), (20, 30)]:
                            _mask = (_step_idx >= _lo) & (_step_idx < _hi)
                            if _mask.sum() > 0:
                                metrics[f'rebel/belief_acc_step_{_lo}_{_hi}'] = float(_r_state[_mask].mean())

                    # 8. Belief drift: mean per-trajectory std of r_state (higher = more unstable belief)
                    if ('rebel_r_state' in batch.non_tensor_batch and
                            'traj_uid' in batch.non_tensor_batch):
                        _r_state = batch.non_tensor_batch['rebel_r_state'].astype(float)
                        _traj_uids = batch.non_tensor_batch['traj_uid']
                        _traj_stds = [
                            float(_np2.std(_r_state[_traj_uids == _t]))
                            for _t in _np2.unique(_traj_uids)
                            if (_traj_uids == _t).sum() >= 2
                        ]
                        if _traj_stds:
                            metrics['rebel/belief_drift'] = float(_np2.mean(_traj_stds))
