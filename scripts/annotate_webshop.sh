...
#
# Usage:
#   bash scripts/annotate_webshop.sh
#
...
python3 "${VERL_AGENT_DIR}/scripts/generate_rebel_sft_data.py" \
    --stage both \
    --env webshop \
    --input     "${VERL_AGENT_DIR}/data/webshop_rebel_sft/sampled_trajectories.json" \
    --annotated "${VERL_AGENT_DIR}/data/webshop_sft_annotated/annotated_trajs.jsonl" \
    --output    "${VERL_AGENT_DIR}/data/webshop_sft/train.jsonl" \
