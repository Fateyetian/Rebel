#!/bin/bash
# =============================================================================
# SFT cold-start pipeline — WebShop
# =============================================================================
# Stages:
...
#
# Usage: bash scripts/sft_webshop.sh
# =============================================================================
...
# ─── Paths ────────────────────────────────────────────────────────────────────
RAW_DATA="${VERL_AGENT_DIR}/data/webshop_rebel_sft/sampled_trajectories.json"
ANNOTATED_DATA="${VERL_AGENT_DIR}/data/webshop_sft_annotated/annotated_trajs.jsonl"
SFT_PAIRS="${VERL_AGENT_DIR}/data/webshop_sft/train.jsonl"
LOCAL_DIR="${VERL_AGENT_DIR}/processed_data/sft/webshop"
CHECKPOINT_DIR="${VERL_AGENT_DIR}/checkpoints/sft/webshop/qwen1.5b_rebel_sft"
EXPERIMENT_NAME=qwen1.5b_rebel_cold-start_webshop
...
if   [ -d "$LOCAL_MODEL"  ]; then MODEL_NAME="$LOCAL_MODEL"
...
if [ -f "$ANNOTATED_DATA" ]; then
    LINES=$(wc -l < "$ANNOTATED_DATA")
    echo ""
    echo "Stage 1: Annotation already exists ($LINES episodes). Skipping."
else
    echo ""
...
if [ -f "$SFT_PAIRS" ]; then
    LINES=$(wc -l < "$SFT_PAIRS")
    echo ""
    echo "Stage 2: SFT pairs already exist ($LINES pairs). Skipping."
else
    echo ""
